...
<head>
    ...
    <!-- Tell the browser to be responsive to screen winvoiceIdth -->
...
<script>
    ...
    function deleteById() {
        var invoiceId = getCheckId()
        if(invoiceId) {
            if(confirm("你确认要删除此条记录吗？")) {
                alert(invoiceId);
                location.href="${ctx}/cargo/invoice/delete.do?id="+invoiceId;
            }
        }else{
    ...
    function submit() {
        var invoiceId = getCheckId()
        if(invoiceId) {
            location.href="${ctx}/cargo/invoice/submit.do?id="+invoiceId;
        }else{
            alert("请勾选待处理的记录，且每次只能勾选一个")
        }
    }
    ...
    function cancel() {
        var invoiceId = getCheckId()
        if(invoiceId) {
            location.href="${ctx}/cargo/invoice/cancel.do?id="+invoiceId;
        }else{
            alert("请勾选待处理的记录，且每次只能勾选一个")
        }
    }
    ...
    function view() {
        var invoiceId = getCheckId()
        if(invoiceId) {
            location.href="${ctx}/cargo/invoice/toView.do?id="+invoiceId;
        }else{
            alert("请勾选待处理的记录，且每次只能勾选一个")
        }
    }
...
<body>
<div invoiceId="frameContent" class="content-wrapper" style="margin-left:0px;">
                ...
                <div class="table-box">
                    ...
                    <div class="pull-left">
                        <div class="form-group form-inline">
                            <div class="btn-group">
                                <button type="button" class="btn btn-default" title="新建" onclick='location.href="${ctx}/cargo/invoice/add.do"'><i class="fa fa-file-o"></i> 新建</button>
                    ...
                    <!--数据列表-->
                    <table invoiceId="dataList" class="table table-bordered table-striped table-hover dataTable">
                        <thead>
                        <tr>
                            <td><input type="checkbox" name="invoiceId" onclick="checkAll('invoiceId',this)"></td>
                        ...
                        <c:forEach items="${page.list}" var="o" varStatus="status">
                            <tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
                                ...
                                <td>
                                    ...
                                    <c:if test="${o.state==2}"><font color="green">已提交</font></c:if>
                                </td>
                                <td>
